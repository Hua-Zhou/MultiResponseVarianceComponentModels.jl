...
B_true = 2rand(rng, p, d) # uniform on [0, 2]
Σ_true = [
    Matrix(UniformScaling(0.2), d, d), 
    Matrix(UniformScaling(0.4), d, d),
...
@testset "fit! (full rank) by MM" begin
    @time hist = MRVC.fit!(mrvc, verbose = true, algo = :MM, maxiter = maxiter)
    println("B_true:")
    display(B_true)
    println()
    println("B̂:")
...
@testset "fit! (full rank) by EM" begin
    @time hist = MRVC.fit!(mrvc, verbose = true, algo = :EM, maxiter = maxiter)
    println("B_true:")
    display(B_true)
    println()
    println("B̂:")
...
Σ_rank = [ones(Int, m - 1); d]
mrvc = MultiResponseVarianceComponentModel(Y, X, V; Σ_rank = Σ_rank)

@testset "fit! (low rank) by MM" begin
    # rank 1 for Σ[1], ..., Σ[m-1], rank d for Σ[m]
    @time hist = MRVC.fit!(mrvc, verbose = true, maxiter = maxiter)
    println("B_true:")
    display(B_true)
    println()
    println("B̂:")
